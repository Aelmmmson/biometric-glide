...
import { FileText, Upload, Scan, X, CreditCard, Vote, Edit, Image as ImageIcon, Eye } from 'lucide-react';
...
import { captureIdentification, searchImages, SearchImagesResponse } from '@/services/api'; // Adjust the import path to match your project structure
import { getRelationNumber } from '@/services/api';
...
type DocumentType = 'national_id' | 'passport' | 'voter_id' | 'drivers_license';
...
export function Identification({ mode = 'capture' }: IdentificationProps) {
  const { state, dispatch } = useBiometric();
  const [captureMode, setCaptureMode] = useState<'scan' | 'upload'>('upload');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editing, setEditing] = useState<{ type: DocumentType; side: 'front' | 'back' } | null>(null);
  const [isAsideOpen, setIsAsideOpen] = useState(false);
  const [images, setImages] = useState<SearchImagesResponse | null>(null);
  const [viewingImage, setViewingImage] = useState<string | null>(null);
  const [currentUploading, setCurrentUploading] = useState<{ type: DocumentType; side: 'front' | 'back' } | null>(null);
  const [isOptionalModalOpen, setIsOptionalModalOpen] = useState(false);
  const [selectedOptionalType, setSelectedOptionalType] = useState<DocumentType | null>(null);
  const [documents, setDocuments] = useState<Record<DocumentType, DocumentSides>>({
    national_id: {},
    passport: {},
    voter_id: {},
    drivers_license: {},
  ...
  useEffect(() => {
    if (mode === 'update') {
      const relationNo = getRelationNumber();
      searchImages(relationNo).then((imgData) => {
        setImages(imgData);
        if (imgData.status === 'success' && imgData.data) {
          ...
          idTypes.forEach((t) => {
            ...
            const unappDoc = imgData.data?.unapproved?.documents?.find((d: DocumentData) => d.type === t.id);
            if (unappDoc) {
              sides = unappDoc.sides;
            } else {
              const appDoc = imgData.data?.approved?.documents?.find((d: DocumentData) => d.type === t.id);
              if (appDoc) {
                sides = appDoc.sides;
              }
            }
  ...
  const handleCardClick = (type: DocumentType, e: React.MouseEvent) => {
  ...
  const handleEditClick = (type: DocumentType, side: 'front' | 'back', e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await captureIdentification(docArray, getRelationNumber());
  ...
  const selectedOptionalInfo = selectedOptionalType ? idTypes.find(t => t.id === selectedOptionalType) : null;

  const currentOptionalDoc = selectedOptionalType ? (documents[selectedOptionalType] || {}) : {};
      ...
      >
        <div className="flex justify-between items-center mb-1">
          <h2 className="text-2xl font-bold">
            {mode === 'update' ? 'Update Identification Document' : 'Identification Document'}
          </h2>
          ...
          >
            ...
            {images.status !== 'success' || !images.data ? (
              ...
              <div className="space-y-6">
                {images.data.unapproved?.documents && images.data.unapproved.documents.length > 0 && (
                  <div>
                    <h4 className="font-semibold mb-2 text-sm uppercase tracking-wide text-muted-foreground">Unapproved</h4>
                    {images.data.unapproved.documents.map((doc: DocumentData) => {
                      const typeInfo = idTypes.find((t) => t.id === doc.type) || { requiresBoth: false };
                      const hasBoth = !!doc.sides.back;
                      const cols = hasBoth ? 'grid-cols-2' : 'grid-cols-1';
                      return (
                        <div key={doc.type} className="mb-4 p-2 border rounded">
                          <h5 className="font-semibold text-xs mb-1 capitalize">{doc.type.replace('_', ' ')}</h5>
                          <div className={`grid ${cols} gap-2`}>
                            {doc.sides.front && getImageSrc(doc.sides.front) && (
                              <div className="space-y-1 relative">
                                <span className="text-xs font-medium text-muted-foreground">Front</span>
                                <div className="relative group">
                                  <img
                                    src={getImageSrc(doc.sides.front)!}
                                    alt={`${doc.type} Front`}
                                    className="w-full aspect-square object-cover rounded border cursor-pointer"
                                    onClick={() => setViewingImage(getImageSrc(doc.sides.front))}
                                  />
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setViewingImage(getImageSrc(doc.sides.front));
                                    }}
                            ...
                            {doc.sides.back && getImageSrc(doc.sides.back) && (
                              <div className="space-y-1 relative">
                                <span className="text-xs font-medium text-muted-foreground">Back</span>
                                <div className="relative group">
                                  <img
                                    src={getImageSrc(doc.sides.back)!}
                                    alt={`${doc.type} Back`}
                                    className="w-full aspect-square object-cover rounded border cursor-pointer"
                                    onClick={() => setViewingImage(getImageSrc(doc.sides.back))}
                                  />
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setViewingImage(getImageSrc(doc.sides.back));
                                    }}
                                    className="absolute top-1 right-1 bg-white/80 hover:bg-white rounded p-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
                                    type="button"
                                    title="View full image"
                ...
                {images.data.approved?.documents && images.data.approved.documents.length > 0 && (
                  <div>
                    <h4 className="font-semibold mb-2 text-sm uppercase tracking-wide text-muted-foreground">Approved</h4>
                    {images.data.approved.documents.map((doc: DocumentData) => {
                      const typeInfo = idTypes.find((t) => t.id === doc.type) || { requiresBoth: false };
                      const hasBoth = !!doc.sides.back;
                      const cols = hasBoth ? 'grid-cols-2' : 'grid-cols-1';
                      return (
