...
export function startTablet(setSigCaptured) {
  try {
    ...
    if (retmod == 11 || retmod == 12 || retmod == 15) {
      ...
      eventTmr = setInterval(SigWebEvent, 50); // Increased from 20ms to 50ms
      console.log(eventTmr);
      tmr = SetTabletState(1, ctx, 100) || tmr; // Increased from 50ms to 100ms
      SetLCDCaptureMode(2);
...
var revokeBlobURL =
  ...
  (window.webkitURL && window.webkitURL.revokeObjectURL.bind(window.webkitURL)) ||
...
function LCDSendGraphicCanvas(dest, mode, x, y, xs, ys, canvas) {
  var Gstr = createLcdBitmapFromCanvas(canvas, 0, 0, xs, ys);
  LcdWriteImageStream(dest, mode, x, y, xs, ys, Gstr);
}

function LCDSendWindowedGraphicCanvas(dest, mode, x, y, xs, ys, canvas, xps, yps) {
